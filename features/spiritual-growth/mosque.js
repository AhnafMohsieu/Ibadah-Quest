...
(function() {
  ...
  function mosqueSVG(stage, progress) {
    ...
    if (stage >= 2) parts += `<rect x="25" y="70" width="70" height="50" fill="var(--gold)" opacity="0.8" rx="3"/>`;
    if (stage >= 3) parts += `<polygon points="18,72 60,38 102,72" fill="#8B4513"/>`;
    if (stage >= 4) {
      parts += `<ellipse cx="60" cy="45" rx="22" ry="18" fill="var(--gold)"/>`;
      parts += `<circle cx="60" cy="28" r="5" fill="#FFD700"/>`;
    }
    if (stage >= 5) {
      parts += `<rect x="95" y="45" width="8" height="75" fill="var(--gold)" rx="2"/>`;
    ...
    if (stage >= 6) {
      ...
      parts += `<rect x="48" y="96" width="24" height="24" fill="var(--gold-light)" opacity="0.25" rx="2"/>`;
    }
    if (stage === 7) {
      parts += `<circle cx="60" cy="70" r="55" fill="none" stroke="var(--gold)" stroke-width="2" opacity="0.4"/>`;
