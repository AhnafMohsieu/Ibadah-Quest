...
test('index.html registers the service worker and update banner', () => {
  assert.ok(html.includes("navigator.serviceWorker.register('sw.js?v=49')"));
...
test('hadith/dhikr audio modules wired with versions and load order', () => {
  ...
  assert.ok(html.includes('styles/main.css?v=34'));
...
test('calm rendering: no fade replay, finite shimmers, no layout transitions', () => {
  ...
  assert.ok(css.includes('pulse 1.2s 3'),
    'audio play pulse must be finite');
  assert.ok(css.includes('muhPulse 2s ease-in-out 3'),
    'pending-entry pulse must be finite');
  assert.ok(css.includes('pulseGlow 1.6s ease-in-out 3'),
    'dhikr play glow must be finite');
